    ...
    void parseMeshPdu(final byte[] pdu) {
        ...
        if (message != null) {
            if (message instanceof AccessMessage) {
                ...
                switch (opCodeLength) {
                    ...
                    case 2:
                        if (message.getOpCode() == ConfigMessageOpCodes.CONFIG_APPKEY_STATUS) {
                            final ConfigAppKeyStatus status = new ConfigAppKeyStatus(mNode, (AccessMessage) message);
                            ...
                            final ConfigModelAppStatus status = new ConfigModelAppStatus(mNode, (AccessMessage) message);
                            if (status.isSuccessful()) {
                            ...
                            final ConfigModelPublicationStatus status = new ConfigModelPublicationStatus(mNode, (AccessMessage) message);
                            if (status.isSuccessful()) {
                    ...
                    case 3:
                        if(mMeshMessage instanceof VendorModelMessageAcked) {
                            ...
                            final VendorModelMessageStatus status = new VendorModelMessageStatus(mNode, (AccessMessage) message, vendorModelMessageAcked.getModelIdentifier());
                            mMeshStatusCallbacks.onMeshMessageReceived(status);
                            ...
                            final VendorModelMessageStatus status = new VendorModelMessageStatus(mNode, (AccessMessage) message, vendorModelMessageUnacked.getModelIdentifier());
                            mMeshStatusCallbacks.onMeshMessageReceived(status);
                        }
                        break;
                    default:
                ...
                parseControlMessage((ControlMessage) message, message.getNetworkPdu().size());
                executeResend();
            }
        } else {
